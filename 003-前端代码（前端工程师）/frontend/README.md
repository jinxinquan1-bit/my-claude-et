...
无构建：**双击任意 HTML 即可在浏览器打开**（推荐从 [index.html](index.html) 登录页开始）。
...
登录/注册、登出、登录态守卫已对接 [../backend](../backend/)（Express + MySQL 8.4）：

```bash
# 终端 1：启动后端（http://localhost:3001）
cd ../backend && npm install && npm start

# 终端 2：预览前端（http://localhost:8080）
python -m http.server 8080
```

- **真实接口**：登录页 → `POST /api/auth/login`、注册 → `POST /api/auth/register`、退出 → `POST /api/auth/logout`、登录态校验 → `GET /api/auth/me`（封装在 [assets/js/api.js](assets/js/api.js)，后端地址在 `API_BASE` 常量）
- **登录态守卫**：所有壳页面未登录/登录过期会自动跳回登录页（common.js）
- **其余页面（学生/记录/统计/报告）仍为演示数据**（mock-data.js），待后端对应接口完成后逐一替换
